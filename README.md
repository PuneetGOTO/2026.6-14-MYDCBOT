...
本專案使用 [GNU Affero General Public License v3.0 or later](LICENSE)（AGPL-3.0-or-later）。

你可以使用、修改、部署與再發布；但如果你修改本專案並透過網路提供機器人或 Web 服務，也必須向使用者提供修改後的完整對應源碼，並保留相同授權。簡單說：改了並拿去運營或提供服務，就要開源修改版。
...
Discord 連線代理可選。只有在本機已啟動 HTTP 代理、且端口確認可用時才填：
...
如果啟動停在 `discord.client: logging in using static token` 後出現 `TimeoutError`，代表這台 Windows 無法穩定連到 Discord API。可以在 `.env` 加入本機 HTTP 代理，例如：

```env
DISCORD_PROXY_URL=http://127.0.0.1:7890
```

請把端口改成你本機代理工具實際正在監聽的 HTTP 代理端口；如果代理沒有啟動，或端口填錯，腳本會在啟動前停止並提示修正。
