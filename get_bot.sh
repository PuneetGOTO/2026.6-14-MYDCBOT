...
CSHARP_SERVICE_NAME="${SERVICE_NAME}-csharp-api"
CSHARP_API_PORT="5088"
...
prompt_input "是否同时部署 C# 控制后端，供 Windows 控制客户端连接？输入 y 启用，其他跳过"
INSTALL_CSHARP_CONTROL="$REPLY"
GJBOT_CONTROL_API_KEY=""
if [[ "$INSTALL_CSHARP_CONTROL" =~ ^[Yy]$ ]]; then
    prompt_secret "请输入 C# 控制 API Key，Windows 客户端需要填写同一个值"
    GJBOT_CONTROL_API_KEY="$REPLY"
fi
...
if [[ "$INSTALL_CSHARP_CONTROL" =~ ^[Yy]$ ]] && [ -z "$GJBOT_CONTROL_API_KEY" ]; then
    log_error "启用 C# 控制后端时，控制 API Key 必填。安装中止。"
    exit 1
fi

# 4. 安装系统级依赖
log_info "正在安装必要的系统依赖..."
apt-get install -y git curl python3-pip python3-venv nginx ffmpeg build-essential certbot python3-certbot-nginx
...
FLASK_SECRET_KEY="$($PYTHON_COMMAND -c 'import secrets; print(secrets.token_hex(32))')"
...
# 8.1 可选：安装并发布 C# 控制后端
if [[ "$INSTALL_CSHARP_CONTROL" =~ ^[Yy]$ ]]; then
    cat > $INSTALL_DIR/csharp-control.env <<EOF
# This file is auto-generated by the installation script.
DISCORD_BOT_TOKEN=${DISCORD_BOT_TOKEN}
GJBOT_CONTROL_API_KEY=${GJBOT_CONTROL_API_KEY}
EOF
    chmod 600 $INSTALL_DIR/csharp-control.env
    log_info "正在安装 .NET 8 SDK 并发布 C# 控制后端..."
    su - $BOT_USER -c "cd $INSTALL_DIR && curl -fsSL https://dot.net/v1/dotnet-install.sh -o dotnet-install.sh"
    su - $BOT_USER -c "cd $INSTALL_DIR && bash dotnet-install.sh --channel 8.0 --install-dir $INSTALL_DIR/.dotnet"
    su - $BOT_USER -c "cd $INSTALL_DIR && $INSTALL_DIR/.dotnet/dotnet publish csharp/src/GJBot.BotServer/GJBot.BotServer.csproj -c Release -o $INSTALL_DIR/csharp-control-server --self-contained false"
    log_success "C# 控制后端发布完成。"
fi
...
cat >> /etc/nginx/sites-available/$SERVICE_NAME <<EOF
    ...
    location /control-api/ {
        proxy_pass http://127.0.0.1:${CSHARP_API_PORT}/;
        proxy_set_header Host \$host;
        proxy_set_header X-Real-IP \$remote_addr;
        proxy_set_header X-Forwarded-For \$proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto \$scheme;
...
if certbot --nginx --non-interactive --agree-tos -d $DOMAIN_NAME -m admin@$DOMAIN_NAME --redirect; then
    ...
    log_error "SSL certificate setup failed. Refusing to run the Web panel, OAuth callback, or payment callback over plain HTTP."
    exit 1
...
ExecStart=$INSTALL_DIR/venv/bin/python3 -m gjbot
Restart=always
RestartSec=10
...
if [[ "$INSTALL_CSHARP_CONTROL" =~ ^[Yy]$ ]]; then
    log_info "正在创建 C# 控制后端 systemd 服务..."
    cat > /etc/systemd/system/$CSHARP_SERVICE_NAME.service <<EOF
[Unit]
Description=GJTeam C# Control API
After=network.target
...
Group=$BOT_USER
WorkingDirectory=$INSTALL_DIR
EnvironmentFile=$INSTALL_DIR/csharp-control.env
ExecStart=$INSTALL_DIR/.dotnet/dotnet $INSTALL_DIR/csharp-control-server/GJBot.BotServer.dll --urls http://127.0.0.1:${CSHARP_API_PORT}
Restart=always
RestartSec=10
...
WantedBy=multi-user.target
EOF
    log_success "C# 控制后端 systemd 服务文件创建成功。"
fi
...
if [[ "$INSTALL_CSHARP_CONTROL" =~ ^[Yy]$ ]]; then
    systemctl enable $CSHARP_SERVICE_NAME
    systemctl start $CSHARP_SERVICE_NAME
fi

log_success "机器人服务已启动！"
if [[ "$INSTALL_CSHARP_CONTROL" =~ ^[Yy]$ ]]; then
    log_success "C# 控制后端已启动！"
fi
...
if [[ "$INSTALL_CSHARP_CONTROL" =~ ^[Yy]$ ]]; then
    echo -e "  - \033[1mWindows 控制客户端后端地址\033[0m: \033[36mhttps://$DOMAIN_NAME/control-api/\033[0m"
    echo -e "  - \033[1m查看 C# 控制后端日志\033[0m: \033[36msudo journalctl -u $CSHARP_SERVICE_NAME -f\033[0m"
    echo -e "  - \033[1m重启 C# 控制后端\033[0m: \033[36msudo systemctl restart $CSHARP_SERVICE_NAME\033[0m"
fi
echo ""
